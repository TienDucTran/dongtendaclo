            ...
            {filteredCourses.map((course) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 flex justify-end gap-3 px-5 py-3 bg-[#F9FAFB]/[0.3] border-t border-[#F3F4F6]">
                  <button onClick={() => handleEditCourse(course)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors" title="Chỉnh sửa">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </button>
                  <button onClick={() => { if (confirm('Bạn có chắc muốn xóa khóa học này?')) { fetch(`/api/admin/courses/${course.id}`, { method: 'DELETE' }).then((res) => { if (res.ok) { setCourses(prev => prev.filter(c => c.id !== course.id)); } }); } }} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Xóa">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
